...
class AgentOrchestrator:
    ...
    async def process_user_turn(self, user_transcript):
        ...
        if self.expire_staged_remediation():
            ...
            'investigate', 'incident brief', 'diagnose', 'diagnos', 'what is causing', 'what caused',
            'what is wrong', 'why is it slow', 'what is the issue', 'what triggered this outage',
            'failure domain', 'triage the current', 'diagnostic triage', 'bottleneck'
        ]):
        ...
        elif any(w in text.lower() for w in ['export', 'download', 'save pdf', 'save docx', 'save report', 'save incident review']) and any(w in text.lower() for w in ['pdf', 'docx', 'word', 'file', 'report', 'document', 'review', 'summary', 'post-mortem', 'postmortem']):
            fmt = 'docx' if any(w in text.lower() for w in ['docx', 'word']) else 'pdf'
            tools = [await self.call_tool('export_incident_report', {'format': fmt})]
            spoken = tools[0]['result'].get('spoken') or tools[0]['result'].get('message', f'Exported report as {fmt.upper()}.')
            ...
            if configured:
                try:
                    ...
                    if not tools and any(w in text.lower() for w in [
                        'restart', 'flush', 'rollback', 'roll back', 'scale', 'failover', 'circuit breaker',
                        'health', 'status', 'overview', 'failing', 'critical', 'degraded', 'infrastructure', 'cluster',
                        'microservice', 'microservices', 'service', 'services', 'alert', 'alerts',
                        'inspect', 'log', 'logs', 'runbook', 'runbooks', 'vitals', 'search', 'news', 'headline', 'headlines', 'breaking',
                        'document', 'documents', 'docs', 'pdf', 'docx', 'export', 'download', 'transcribe', 'recording',
                        ...
                        'recovery plan', 'recovery steps', 'optimal', 'sop', 'procedure', 'procedures', 'walk me through', 'step through',
                        'topology', 'dependency', 'dependencies', 'graph', 'up', 'online', 'running', 'alive',
                        'why', 'timeout', 'timeouts', 'timing out', 'slow', 'spike', 'dump', 'fail', 'api', 'requests'
                    ]):
    ...
    def _mutation_clarification(self, text):
        ...
        if any(w in lower for w in [
            'memory', 'search', 'look up', 'retrieve', 'history', 'past', 'what happened',
            'tell me about', 'tree of thoughts', 'tot', 'mitigation', 'simulate', 'simulation', 'plan', 'risk'
        ]):
            return None
        ...
        aliases = [
            ...
            ('database', 'order-db'), ('postgres', 'order-db'), ('db', 'order-db'),
            ('ingress', 'ingress-gateway'), ('gateway', 'ingress-gateway'),
    ...
    async def _deterministic_agent_reasoning(self, text):
        ...
        if not target:
            target = next((sid for word, sid in [
                ('redis', 'redis-cache'), ('cache', 'redis-cache'),
                ('database', 'order-db'), ('postgres', 'order-db'), ('db', 'order-db'),
                ('ingress', 'ingress-gateway'), ('gateway', 'ingress-gateway'),
                ('payment', 'payment-service')
            ] if word in lower), 'payment-service')
        ...
        if any(w in lower for w in [
            'what is wrong', 'why is it slow', 'what is the issue', 'diagnosis', 'diagnostics',
            'diagnos', 'what should we do', 'recommendation', 'going wrong', 'failure domain',
            'triage', 'root cause anomalies', 'brief me', 'incident brief', 'active sev-1', 'active incident',
            'investigat', 'investigation', 'degrading', 'bottleneck', 'what triggered'
        ]) and 'runbook' not in lower:
        ...
        if any(w in lower for w in [
            'causal', 'root cause', 'microhecl', 'localize cause', 'what is the root cause',
            'source of', 'originate', 'originat', 'fault', 'causal graph', 'causal blame',
            'causal trigger', 'causal inference', 'causal discovery', 'highest root cause'
        ]):
            name, args = 'locate_causal_root_cause', {}
        elif any(w in lower for w in [
            'incident memory', 'retrieve memory', 'search memory', 'past incidents', 'episodic memory',
            'what do you remember', 'recall memories', 'memory for', 'memory regarding', 'memories regarding',
            'look up memory', 'search episodic', 'memory bank', 'query incident memory'
        ]) and not any(w in lower for w in ['host memory', 'pc memory', 'machine memory', 'high memory', 'out of memory', 'memory leak']):
            query_q = re.sub(
                r'^(?:retrieve|search|find|recall|look up|query)?\s*(?:incident|episodic)?\s*memor(?:y|ies)\s*(?:bank\s+)?(?:for|regarding|about)?|past\s+incidents?\s*(?:about|for)?',
                '',
                lower
            ).strip(' ?!.,')
            name, args = 'retrieve_incident_memory', {'query': query_q or 'connection pool failure'}
        elif any(w in lower for w in [
            'dejavu', 'déjàvu', 'deja vu', 'déjà vu', 'historical incident', 'match incident',
            'recurring incident', 'similar incident', 'similar past', 'past incident', 'recurring pattern',
            'hist-', 'similarity score', 'remediations worked', 'failed like this', 'dejavu database',
            'déjàvu database', 'dejavu index', 'déjàvu index', 'recurring'
        ]):
            name, args = 'match_historical_incident', {}
        elif any(w in lower for w in [
            'plan mitigation', 'tree of thoughts', 'mitigation tree', 'simulate remediation',
            'simulate plan', 'decision tree', 'mitigation trajectory', 'mitigation trajectories',
            'tot', 'remediation plan', 'recovery plan', 'mitigation plan', 'remediation sequence',
            'mitigation path', 'mitigation paths', 'mitigation outcome', 'optimal recovery',
            'recover the cluster fastest', 'recover fastest', 'recovery steps', 'risk-benefit',
            'tradeoff in tot', 'optimal remediation', 'remediation options', 'projected value scores',
            'mitigation decision', 'mitigation branches', 'mitigation options', 'pareto',
            'simulate mitigation'
        ]):
            name, args = 'plan_mitigation_tree', {}
        elif 'runbook' in lower or any(w in lower for w in ['step', 'advance', 'next step', 'execute step', 'sop', 'standard operating procedure', 'walk me through', 'step through']):
            if any(w in lower for w in ['abort', 'cancel', 'stop', 'exit']) and any(w in lower for w in ['runbook', 'sop', 'procedure']):
                name = 'abort_runbook'
            elif any(w in lower for w in ['list', 'available', 'show', 'what standard operating', 'overview']) and any(w in lower for w in ['runbook', 'sop', 'procedure', 'operating', 'runbooks']):
                name = 'list_runbooks'
            elif any(w in lower for w in ['next', 'advance', 'proceed', 'continue', 'remain', 'confirm and advance', 'current step', 'active runbook action', 'runbook action']) or lower in {'next step', 'execute step', 'advance', 'step'}:
                name = 'advance_runbook'
            else:
                name = 'start_runbook'
                args = {'runbook_id': 'runbook-redis-eviction' if any(w in lower for w in ['redis', 'memory']) else ('runbook-ingress-surge' if any(w in lower for w in ['ingress', 'gateway', 'surge']) else 'runbook-pg-pool')}
        elif any(w in lower for w in ['restart', 'flush', 'rollback', 'roll back', 'scale', 'failover', 'circuit breaker']) and not any(w in lower for w in ['search', 'look up', 'google', 'duckduckgo', 'find', 'example', 'memory', 'history', 'what is', 'why']):
        ...
        elif any(w in lower for w in ['export', 'download', 'save report', 'generate signed pdf', 'downloadable word']) and any(w in lower for w in ['pdf', 'docx', 'word', 'report', 'post-mortem', 'postmortem', 'summary', 'review']):
            fmt = 'docx' if any(w in lower for w in ['docx', 'word']) else 'pdf'
            name, args = 'export_incident_report', {'format': fmt}
        elif any(w in lower for w in ['inspect document', 'read document', 'inspect file', 'read file', 'inspect pdf', 'read pdf', 'inspect docx', 'read docx']) or (any(w in lower for w in ['inspect', 'read']) and any(w in lower for w in ['.md', '.pdf', '.docx', '.txt', 'docs/'])):
            ...
            'list all the items', 'document repository', 'docs directory', 'files are in my document',
            'documents in docs', 'documents available', 'docs are present', 'documents in my repo',
            'check files in docs', 'show me documents'
        ]) or (any(w in lower for w in ['list', 'show', 'what files', 'what docs', 'check files']) and any(w in lower for w in ['document', 'documents', 'docs', 'files in docs'])):
            match_dir = re.search(r'(?:in|from)\s+(?:the\s+|my\s+)?([^\s]+)\s+(?:folder|directory|repo)?', lower)
            dir_name = match_dir.group(1) if match_dir else 'docs'
            if dir_name in ['document', 'documents', 'my', 'the']:
                dir_name = 'docs'
        ...
        elif any(w in lower for w in ['topology', 'dependency', 'dependencies', 'depend on', 'depends on', 'dependency graph', 'blast radius']):
            name = 'get_service_topology'
        elif 'log' in lower or 'why' in lower:
            name, args = 'inspect_service_logs', {'service_name': target}
        elif any(w in lower for w in ['host', 'pc ', 'machine']) and any(w in lower for w in ['cpu', 'memory', 'metric', 'telemetry', 'ram', 'swap', 'load', 'stats', 'performance']):
            ...
            'cpu', 'memory', 'metric', 'metrics', 'telemetry', 'latency', 'error rate', 'error_rate',
            'golden signal', 'golden signals', 'saturation', 'throughput', 'traffic', 'load', 'vitals',
            'signals', 'sla', 'slo'
        ]):
        ...
        elif not any(w in lower for w in [
            'health', 'alert', 'alerts', 'status', 'failing', 'overview', 'critical', 'degraded',
            'infrastructure', 'cluster', 'microservice', 'microservices', 'service', 'services',
            'operational', 'all clear', 'green', 'doing', 'up', 'online', 'running', 'alive', 'ok',
            'database', 'redis', 'payment', 'ingress', 'gateway', 'system'
        ]):
